  ...
  var DATA =  getProgressData()
    ...
    const renderItemTranslate = ({item}) => {
      ...
      if (item.questionType == "translate") {
          ...
          return (
              <View style={{backgroundColor: palette.interactable, marginBottom: 10}}>
                  <View style={styles.blueListItem}><HebrewText style={styles.body}>{item.depiction}</HebrewText><HebrewText style={styles.body}>{item.timeRemaining + "s"}</HebrewText><HebrewText style={styles.body}>{(item.correctAnswer ? `\u{1F44D}` : `\u{1F44E}`)}</HebrewText><HebrewText style={styles.body}>{item.guesses}</HebrewText></View>
